(function( $, undefined ) {
	...
	$( document ).on("pagebeforechange", function( e,f ){
		...
		if( typeof hash !== "undefined" &&
			...
			hash !== "" &&
			hashEl.length > 0 &&
			!hashEl.hasClass( "page" ) &&
			!hashEl.hasClass( "popup" ) &&
			hashEl.data('role') !== "page" &&
			!hashElInPage.hasClass( "panel" ) &&
			!hashElInPage.hasClass( "popup" ) ) {
			//scroll to the id
			...
			!hashEl.hasClass( "page" ) && hashEl.attr('data-role') !== "page" &&
			!hashElInPage.hasClass( "panel" ) &&
			!hashElInPage.hasClass( "popup" ) ) {
			$( ele ).attr( "href", href );
	...
	$( document ).on( "mobileinit", function(){
		$.widget( "mobile.h2linker", {
			...
			_create:function(){
				var self = this,
					bodyid = "page-top",
					panel = "<div data-role='panel' data-role='jqm-nav-panel jqm-quicklink-panel' data-position='right' data-display='overlay' data-theme='a'><ul data-role='listview' data-inset='false' data-theme='a' data-divider-theme='a' data-icon='false' data-role='jqm-list'><li data-role='list-divider'>Quick Links</li></ul></div>",
					first = true,
					...
					this.element.find("div.jqm-content>h2").each(function(){
						...
						if(!first){
							$(this).before( "<a href='#" + bodyid + "' data-role='jqm-deeplink icon-carat-u alt-icon'>Top</a>");
						} else {
							$(this).before("<a href='#' data-ajax='false' data-role='jqm-deeplink jqm-open-quicklink-panel icon-carat-l alt-icon'>Quick Links</a>");
						}
						first = false;
					});
					this._on(".jqm-open-quicklink-panel", {
						"click": function(){
							$(".page-active .jqm-quicklink-panel").panel("open");
							return false;
						}
					});
					this._on( document, {
						"pagebeforechange": function(){
							...
							this.element.find(".jqm-quicklink-panel .btn-active").removeClass("btn-active");
						}
					});
...
$( document ).on( "pagecreate", ".jqm-demos", function( event ) {
	var search,
		...
		searchContents = $( ".jqm-search ul.jqm-list" ).find( "li:not(.collapsible)" ),
...
$( document ).one( "pagecreate", ".jqm-demos", function( event ) {
	$( this ).find( ".jqm-search-results-list li, .jqm-search li" ).each(function() {
		...
		$( this )
			.find( "a" )
			.append( "<span data-role='jqm-search-results-keywords li-desc'>" + text + "</span>" );
	});
});
...
$( document ).on( "mobileinit", function() {
	...
	$.widget( "mobile.listview", $.mobile.listview, {
		...
		enterToNav: function() {
			...
			form.append( "<button type='submit' data-icon='carat-r' data-inline='true' data-role='hidden-accessible' data-iconpos='notext'>Submit</button>" )
				.parent()
				.trigger( "create" );

			this.element.parent().find( "form" ).children( ".btn" ).addClass( "hidden-accessible" );
		...
		handleKeyUp: function( e ) {
			...
			if ( e.which === $.ui.keyCode.DOWN ) {
				if ( this.element.find( "li.btn-active" ).length === 0 ) {
					this.element.find( "li:first" ).toggleClass( "btn-active" ).find("a").toggleClass( "btn-active" );
				} else {
					this.element.find( "li.btn-active a" ).toggleClass( "btn-active");
					this.element.find( "li.btn-active" ).toggleClass( "btn-active" ).next().toggleClass( "btn-active" ).find("a").toggleClass( "btn-active" );
				}
				...
				if ( this.element.find( "li.btn-active" ).length !== 0 ) {
					this.element.find( "li.btn-active a" ).toggleClass( "btn-active");
					this.element.find( "li.btn-active" ).toggleClass( "btn-active" ).prev().toggleClass( "btn-active" ).find("a").toggleClass( "btn-active" );
				} else {
					this.element.find( "li:last" ).toggleClass( "btn-active" ).find("a").toggleClass( "btn-active" );
				}
				this.highlightUp();
			} else if ( typeof e.which !== "undefined" ) {
				this.element.find( "li.btn-active" ).removeClass( "btn-active" );
		...
		submitHandler: function() {
			if ( this.element.find( "li.btn-active" ).length !== 0 ) {
				var href = this.element.find( "li.btn-active a" ).attr( "href" );
		...
		highlightDown: function() {
			if ( this.element.find( "li.btn-active" ).hasClass( "screen-hidden" ) ) {
				this.element.find( "li.btn-active" ).find("a").toggleClass( "btn-active" );
				this.element.find( "li.btn-active" ).toggleClass( "btn-active" ).next().toggleClass( "btn-active" ).find("a").toggleClass( "btn-active" );
				this.highlightDown();
			}
			return;
	...
	popup.one( "popupbeforeposition", function() {
		...
		$.each( sources, function( idx, options ) {
			collapsible = $( "<div data-role='collapsible' data-collapsed='true' data-theme='" + options.theme + "' data-iconpos='right' data-collapsed-icon='carat-l' data-expanded-icon='carat-d' data-content-theme='b'>" +
					"<h1>" + options.title + "</h1>" +
					"<pre data-role='brush: " + options.brush + ";'></pre>" +
				"</div>" );
...
$( document ).bind( "pagebeforechange", function( e, data ) {
	...
	if ( data.options && data.options.role === "popup" && data.options.link ) {
		...
		if ( sources ) {
			popup = $( "<div id='jqm-view-source' data-role='jqm-view-source' data-role='popup' data-theme='none' data-position-to='window'>" +
...
function makeButton() {
	...
	a.className = "jqm-view-source-link btn corner-all btn-inline mini";
...
$.fn.viewSourceCode = function() {
	return $( this ).each( function() {
		var button = makeButton(),
			self = $( this ),
			snippetSource = self.parents( ".page,:jqmData(role='page')" ).add( $( "head" ) ),
...
$( document ).on( "pagecreate", function( e ) {
	...
	$( document ).on( "mousewheel", ".jqm-view-source .collapsible-content", function( event, delta ) {
	...
	$( e.target ).delegate( ".jqm-view-source .collapsible", "expand", function() {
	...
	$( e.target ).delegate( ".jqm-view-source", "popupbeforeposition", function() {
		...
		var x = $( this ).find( ".collapsible" ).length,
			maxHeight = $.mobile.getScreenHeight() - 60 - ( x * 42 );

		$( this ).find( ".collapsible-content" ).css( "max-height", maxHeight + "px" );

		// keep line numbers and code lines in sync
		$(".collapsible:not(.collapsible-collapsed) .gutter", this ).find( ".line" ).css( "height", "");

		$(".collapsible:not(.collapsible-collapsed) .code", this ).find( ".line" ).each( function() {
			if ( $( this ).height() !== 16 ) {
...
;(function()
{
	...
	function Brush()
	{
		...
		var keywords =	'abstract and array as break case catch cfunction data-role clone const continue declare default die do ' +
