 ...
 *  	attribute data-marker-info="lat,lng". If the element or any descendant has the data-role
 *  	"gmap-marker-content", then an infobox containing that content will be shown when
 *  	the marker is clicked (this content is cloned). The HTML contents of a subelement with
 *  	data-role "gmap-marker-title" will be used for the mouseover tooltip.
 *  infoWindowConfig: Object containing a list of properties for the info window (if applicable).
 *
 * Markup requirements:
 *
 *  1. The data-role "gmap" will be added to each element to which the plugin is applied.
 */
(function($) {
	var pluginName = 'gmap';
	...
	var commands = {
		init: function(config) {
			...
			// Set up UI
			$this.addClass('gmap').html('<div data-role="gmap-canvas"></div>');
			...
			var $canvas = $this.find('.gmap-canvas');
			...
			$('[data-marker-info]').each(function() {
				try {
					...
					data.push({ position: pos,
						title: $(this).find('.gmap-marker-title').html(),
						content: $(this).find('.gmap-marker-info').andSelf().filter('.gmap-marker-info').clone().get(0)
					});
				}
				catch (e) {
					console.log(e);
